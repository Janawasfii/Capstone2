...
public class WorkoutsController {
    ...
    @PostMapping("/add")
    public ResponseEntity addWorkouts(@Valid @RequestBody Workouts workouts, Errors errors){
        if(errors.hasErrors()){
    ...
    public ResponseEntity updateWorkouts(@PathVariable Integer id, @Valid @RequestBody Workouts workouts, Errors errors){
        if(errors.hasErrors()){
            String message = errors.getFieldError().getDefaultMessage();
            return ResponseEntity.status(400).body(message);
        }
