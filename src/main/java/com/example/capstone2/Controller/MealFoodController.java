...
public class MealFoodController {
    ...
    @PostMapping("/add")
    public ResponseEntity addMealFood(@Valid @RequestBody MealFood mealFood, Errors errors){
        if(errors.hasErrors()){
    ...
    public ResponseEntity updateMealFood(@PathVariable Integer id, @Valid @RequestBody MealFood mealFood, Errors errors){
        if(errors.hasErrors()){
            String message = errors.getFieldError().getDefaultMessage();
            return ResponseEntity.status(400).body(message);
        }
