...
public class FoodController {
    ...
    @PostMapping("/add")
    public ResponseEntity addFood(@Valid @RequestBody Food food, Errors errors){
        if(errors.hasErrors()){
    ...
    public ResponseEntity updateFood(@PathVariable Integer id, @Valid @RequestBody Food food, Errors errors){
        if(errors.hasErrors()){
            String message = errors.getFieldError().getDefaultMessage();
            return ResponseEntity.status(400).body(message);
        }
