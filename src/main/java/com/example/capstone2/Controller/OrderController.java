...
public class OrderController {
    ...
    @PostMapping("/add")
    public ResponseEntity addOrder(@Valid @RequestBody Orders orders, Errors errors){
        if(errors.hasErrors()){
    ...
    public ResponseEntity updateOrder(@PathVariable Integer id, @Valid @RequestBody Orders orders, Errors errors){
        if(errors.hasErrors()){
            String message = errors.getFieldError().getDefaultMessage();
            return ResponseEntity.status(400).body(message);
        }
