...
public class RestaurantsController {
    ...
    @PostMapping("/add")
    public ResponseEntity addRestaurants(@Valid @RequestBody Restaurants restaurants, Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity updateRestaurants(@PathVariable Integer id, @Valid @RequestBody Restaurants restaurants, Errors errors) {
        if (errors.hasErrors()) {
            String message = errors.getFieldError().getDefaultMessage();
            return ResponseEntity.status(400).body(message);
        }
