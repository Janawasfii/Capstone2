...
public class ScheduleController {
    ...
    @PostMapping("/add")
    public ResponseEntity addSchedule(@Valid @RequestBody Schedule schedule, Errors errors){
        if(errors.hasErrors()){
    ...
    public ResponseEntity updateSchedule(@PathVariable Integer id, @Valid @RequestBody Schedule schedule, Errors errors){
        if(errors.hasErrors()){
            String message = errors.getFieldError().getDefaultMessage();
            return ResponseEntity.status(400).body(message);
        }
